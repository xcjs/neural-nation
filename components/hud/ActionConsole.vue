...
<template>
  <div class="border border-cyan-900/50 bg-black/70 backdrop-blur-sm p-3">
    ...
    <div class="space-y-1 max-h-64 overflow-y-auto">
      ...
      <div v-if="actions.items.length === 0" class="text-cyan-700 text-xs">
        No actions logged.
      </div>
    </div>
    <div v-if="actions.total > actions.pageSize" class="flex items-center justify-between mt-2 text-xs">
      <button :disabled="actions.page === 0" class="text-cyan-500 disabled:opacity-30" @click="actions.fetchPage(token, actions.page - 1)">
        ← PREV
      </button>
      <span class="text-cyan-700">{{ actions.page + 1 }}/{{ Math.ceil(actions.total / actions.pageSize) }}</span>
      <button :disabled="(actions.page + 1) * actions.pageSize >= actions.total" class="text-cyan-500 disabled:opacity-30" @click="actions.fetchPage(token, actions.page + 1)">
        NEXT →
      </button>
    </div>
  </div>
