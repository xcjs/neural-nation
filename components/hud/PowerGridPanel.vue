...
<template>
  <div class="border border-cyan-900/50 bg-black/70 backdrop-blur-sm p-3">
    ...
    <div v-if="!power.grid" class="text-cyan-700 text-xs">
      No power grid data.
    </div>
    <div v-else class="space-y-1 text-xs">
      <div class="flex justify-between">
        <span class="text-cyan-700">Status:</span><span :class="gridStatusColor">{{ power.grid.status }}</span>
      </div>
      <div class="flex justify-between">
        <span class="text-cyan-700">Capacity:</span><span class="text-cyan-300">{{ power.grid.totalCapacity.toFixed(0) }} MW</span>
      </div>
      <div class="flex justify-between">
        <span class="text-cyan-700">Demand:</span><span class="text-cyan-300">{{ power.grid.totalDemand.toFixed(0) }} MW</span>
      </div>
      <div class="flex justify-between">
        <span class="text-cyan-700">Grids:</span><span class="text-cyan-300">{{ power.grid.connectedGrids.length }}</span>
      </div>
