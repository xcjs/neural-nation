<template>
  <div class="border border-cyan-900/50 bg-black/70 backdrop-blur-sm p-3">
    <h3 class="text-cyan-400 text-xs font-bold tracking-wider mb-2">ENVIRONMENT</h3>
    ...
    <div class="mt-2 flex gap-1">
      <button
        ...
        :class="['text-xs px-2 py-0.5 border', env.showPollutionHeatmap ? 'border-red-500 text-red-300' : 'border-cyan-900 text-cyan-700']"
      >
        POLLUTION MAP
      </button>
      <button
        @click="env.showBiomeDegradation = !env.showBiomeDegradation"
        :class="['text-xs px-2 py-0.5 border', env.showBiomeDegradation ? 'border-green-500 text-green-300' : 'border-cyan-900 text-cyan-700']"
      >
        BIOME MAP
      </button>
    </div>
