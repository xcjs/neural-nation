...
const CLIMATE_FOREST_DENSITY: Record<string, number> = {
  Af: 1.0, Am: 1.0, As: 0.5, Aw: 0.5,
  BSh: 0.1, BSk: 0.1, BWh: 0.0, BWk: 0.0,
  Cfa: 0.8, Cfb: 0.8, Cfc: 0.7,
  Csa: 0.5, Csb: 0.5, Csc: 0.4,
  Cwa: 0.5, Cwb: 0.5, Cwc: 0.4,
  Dfa: 0.6, Dfb: 0.6, Dfc: 0.55, Dfd: 0.5,
  Dsa: 0.4, Dsb: 0.4, Dsc: 0.35, Dsd: 0.3,
  Dwa: 0.4, Dwb: 0.4, Dwc: 0.35, Dwd: 0.3,
  ET: 0.15, EF: 0.0,
}

function climateToForestDensity(code: string): number {
  if (CLIMATE_FOREST_DENSITY[code] !== undefined) return CLIMATE_FOREST_DENSITY[code]!
  if (code.length >= 1) {
    const first = code[0]!
    if (first === 'A') return 0.7
    if (first === 'B') return 0.05
    if (first === 'C') return 0.6
    if (first === 'D') return 0.4
    if (first === 'E') return 0.05
  }
  return 0.3
}
